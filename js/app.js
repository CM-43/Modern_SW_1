...
(function () {
  ...
  function compactCardHTML(microbe, theSite, carried) {
    ...
    return '<div class="compact' + (carried ? " carried" : "") + '" data-compact="' + esc(name) + '">' +
      '<div class="compact-head" data-action="toggle-compact">' +
      ...
      '<div class="compact-body" hidden>' + statsHTML(microbe, theSite) + '</div>' +
      '</div>';
  }
  ...
  function step1HTML(forPreview) {
    ...
    var workspace =
      '<div class="workspace"><div class="step1">' +
        ...
        '</div>' +
        '<div class="step1-hint">Switch on exactly two of the seven characteristics.' +
        (ui.on.length ? ' <br><br>Chosen: ' + esc(ui.on.join(", ")) + '.' : '') +
        '</div>' +
      '</div></div>';
  ...
  function paintSlider(attribute) {
    ...
    selected.style.left = pct(range[0]) + "%";
    selected.style.width = (pct(range[1]) - pct(range[0])) + "%";
    var from = wrap.querySelector(".bubble-from");
    var to = wrap.querySelector(".bubble-to");
    from.textContent = range[0];
    to.textContent = range[1];
    from.style.left = pct(range[0]) + "%";
    to.style.left = pct(range[1]) + "%";
  }
  ...
  function step5HTML(forPreview) {
    ...
    var workspace = '<div class="workspace"><div class="columns three">' +
      ...
      destinationColumn("Returned", MARKING.RETURN) + '</div></div>';
  ...
  function step34HTML() {
    ...
    var workspace = '<div class="workspace"><div class="s34">' + top +
      '<div class="s34-bottom"><div class="grid">' + cells + '</div></div>' +
      '</div></div>';
  ...
  app.addEventListener("click", function (event) {
    ...
    switch (action) {
      ...
      case "toggle-compact": {
        var body = target.parentElement.querySelector(".compact-body");
